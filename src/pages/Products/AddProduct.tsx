...
interface ProductFormData {
  ...
  isActive?: boolean;
  brandName?: string; // For display only
}
...
export default function AddProduct() {
  ...
  const fetchBrands = async () => {
    if (!token) {
      console.log("No token available for brands fetch");
      return;
    }

    setBrandsLoading(true);
    try {
      console.log("Fetching brands with token...");
      const response = await fetch(
      ...
      if (data.success) {
        setBrands(data.data);
        console.log(`${data.data.length} brands loaded successfully`);
      } else {
        console.error("Failed to fetch brands:", data);
        // Try without auth as fallback
        const publicResponse = await fetch(
          "https://gamersbd-server.onrender.com/api/brands?isActive=true",
        );
        const publicData: ApiResponse = await publicResponse.json();
        if (publicData.success) {
          setBrands(publicData.data);
          console.log(`${publicData.data.length} brands loaded (public)`);
        }
      }
    } catch (err) {
  ...
  // Replace your current handleNumberChange with this:
  const handleNumberChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const { name, value } = e.target;

    if (name.includes(".")) {
      // Handle nested objects like ageRange.min, players.max, etc.
      const [parent, child] = name.split(".");
      setFormData((prev) => ({
        ...prev,
        [parent]: {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const productData = {
        ...
        brandName: undefined, // Remove from final payload
      };
